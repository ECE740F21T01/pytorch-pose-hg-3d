...
By changing to load the MPI-INF-3DHP (mpii3d) dataset from "data/vibe_db/mpii3d_train_db.pt" or "data/vibe_db/mpii3d_val_db.pt"
...
class MPII3D(data.Dataset):
  ...
  def _get_part_info(self, index):
    ...
    pts = np.array(ann['joints3D_image'], np.float16) # joint_3d_image is [image_coord_x, image_coord_y, depth-root depth], in mpii format (16 joints)
